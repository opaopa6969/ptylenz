...
flowchart TB
    ...
    Term[ターミナルエミュレータ]
    Wait["wait(101)<br/>← bash はここで寝てるだけ"]
    Bash --> Fork
    Bash --> Wait
    Fork --> Child
...
## 付録: オプション依存（feature gate）の取り入れ方

重い依存 or 「品質向上だけの依存」をオプション化したい時の指針。C の `#ifdef` と同じ発想を、Rust では **Cargo features + `#[cfg(...)]`** で実現する。
...
- `optional = true` で「feature が有効な時だけリンク」
- `dep:unicode-segmentation` は「この feature を有効にしたら同名依存を引き込む」書式（Rust 1.60+）
...
```bash
cargo build                                    # default のみ
cargo build --features grapheme-truncate       # feature 有効化
cargo build --no-default-features              # default feature も無効
```
...
| 条件付き `#include` | `Cargo.toml` の `optional = true` |

`#[cfg]` は feature 以外も分岐できる:

```rust
#[cfg(target_os = "linux")]
#[cfg(target_arch = "x86_64")]
#[cfg(debug_assertions)]    // debug ビルドだけ
#[cfg(test)]                // cargo test の時だけ
#[cfg(all(unix, not(target_os = "macos")))]
```

関数・モジュール・struct メンバー・use 文など粒度細かく付けられる。

### ptylenz で feature を切る判断基準

- **切る**: 重量級依存 / 選択肢がある依存（例: clipboard 実装の切替）/ プラットフォーム固有の実装
- **切らない**: 軽量で実害ないもの → デフォで入れて分岐を書かない方が保守しやすい
- feature 組み合わせはテストマトリクスを爆発させるので、**数は少ないほどよい**

現時点で feature 化したいもの: なし。`unicode-segmentation`（ZWJ / grapheme cluster 対応）は軽量なので feature 化せず default 依存として入れた。
