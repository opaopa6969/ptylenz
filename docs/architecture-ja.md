...
ファイルシステム監視用の `notify` クレートは初期に足場として入れたものの一度も使われず（上記の通りフィーダはポーリングする）、現在は `Cargo.toml` から削除済み。経緯と、将来のローカル専用モードで再導入する場合の方針は [docs/decisions/notify-dead-dep.md](decisions/notify-dead-dep.md) を参照。
...
> `notify` クレートは初期に足場として入れたが一度も使われず、現在は削除済み。詳細は [decisions/notify-dead-dep.md](decisions/notify-dead-dep.md) を参照。
