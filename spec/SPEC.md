...
graph TB
    ...
    subgraph Ptylenz["ptylenz プロセス"]
        INPUT["handle_input()"]
        PTY_WRITE["PtyProxy::write_input()"]
        PTY_MASTER["PTY master fd"]
    ...
    subgraph Shell["bash 子プロセス (PTY slave)"]
        BASH["bash -i --rcfile"]
    end

    STDIN --> INPUT
    INPUT --> PTY_WRITE
    PTY_WRITE --> PTY_MASTER
    PTY_MASTER --> BASH
    BASH --> PTY_MASTER
    PTY_MASTER --> PTY_READ
    PTY_READ --> FEED
    FEED --> OscParser_box
    OscParser_box --> CLEAN
    OscParser_box --> EVENTS
    CLEAN --> STDOUT
    EVENTS --> HISTORY
    HISTORY --> TUI
    JSONL --> MPSC
    MPSC -->|"ingest_claude_event()"| HISTORY
```
...
#### Block / BlockSource クラス図 (Mermaid)
...
classDiagram
    class Block {
        ...
        +DateTime~Local~ started_at
        +Option~DateTime~Local~~ ended_at
        +bool collapsed
        +bool pinned
        +BlockSource source
        +Option~String~ rendered_text
        +usize cached_line_count
        +line_count() usize
        +output_text() String
    }
    ...
    class ClaudeTurnData {
        +String session_id
        +String role
        +usize turn_index
        +Vec~ToolUse~ tool_uses
    }
    ...
    class BlockEngine {
        +Vec~Block~ blocks
        +usize next_id
        +Option~Block~ current
        +OscParser osc_parser
        +feed_output(bytes Vec~u8~)
        +ingest_claude_event(event ClaudeEvent)
        +search(query str) Vec~SearchHit~
        +export_json() String
        +toggle_collapse(id usize)
        +toggle_pin(id usize)
        +get_block_by_index(index usize) Option~Block~
    }
    ...
    class OscParser {
        ...
        +Vec~u8~ buf
        +Vec~u8~ pending
        +parse(bytes Vec~u8~) Vec~ParseChunk~
    }

    Block --> BlockSource : source
    BlockSource --> ClaudeTurnData : ClaudeTurn variant
    ClaudeTurnData --> ToolUse : tool_uses
    BlockEngine --> Block : blocks / current
    BlockEngine --> OscParser : osc_parser
```
...
stateDiagram-v2
    ...
    note right of OPEN
        line_count > 50 の場合
        CommandEnd 時に collapsed = true
    end note

    note right of CLOSED
        ClaudeTurn ブロックは
        OPEN を経ずに直接 CLOSED へ
        (ingest_claude_event)
    end note
```
...
#### PROMPT_COMMAND injection + claude_feeder polling シーケンス図 (Mermaid)
...
sequenceDiagram
    ...
    loop 400ms ポーリング
        feeder->>jsonl: mtime チェック / tail
        jsonl-->>feeder: 新しい行
        feeder->>ptylenz: tx.send(ClaudeEvent::Turn {...})
    end

    Note over User,bash: ユーザーがコマンドを入力
    User->>ptylenz: stdin: "ls -la\n"
    ptylenz->>bash: PTY write: "ls -la\n"

    bash-->>ptylenz: PS0 展開 → OSC 133;C (CommandStart)
    ptylenz->>ptylenz: BlockEngine: Block::new(id=N)
...
`--shell` に渡すパスは起動前に検証する。存在しない・ディレクトリ・実行権限なしの場合は fork 前にエラー終了する (`/` を含まない名前は `execvp` と同じく `$PATH` から解決する)。`$SHELL` が空文字列の場合は未設定と同じく `/bin/bash` にフォールバックする。
